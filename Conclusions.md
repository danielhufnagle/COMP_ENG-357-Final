The current implementation obviously does not utilize the GPU as this is still a work in progress. Thus no concrete conclusions about the speed increase of a parallel implementation of Held's algorithm can be made. Our current code that expands the pseudocode from section 3 to fully implement Held's algorithm on the GPU in OpenROAD is shown below. Currently, we are in the process of modifying the makefile to get OpenROAD to compile with the CUDA code to both test that the implementation functions as intended as well as benchmarking it. The current GPU implementation is shown below.
